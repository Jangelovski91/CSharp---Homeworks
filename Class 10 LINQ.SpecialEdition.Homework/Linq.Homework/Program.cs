    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            Console.WriteLine("1.======= Songs starts with A ======================================");
            var songsStartsWithA = Songs
                                    .Where(s => s.Name.StartsWith("A"))
                                    .ToList();
            songsStartsWithA.ForEach(s => Console.WriteLine(s.Name));
            // 2- how many artists end with letter 'a' (case insensitive)
            Console.WriteLine("2.======= Artists ends with A ======================================");

            var artistsEndWithA = Artists
                                    .Where(a => a.FullName.EndsWith("a"))
                                    .ToList();
            artistsEndWithA.ForEach(a => Console.WriteLine(a.FullName));

            //3 - whats the name of the song with longest duration
            Console.WriteLine("3.======= Song with longest duration ======================================");
            var songsDuration = Songs
                                .Select(s => s.Duration)
                                .ToList();
            var longestSong = songsDuration
                                       .Max();
            var longestSongName = Songs
                                    .Where(s => s.Duration == longestSong)
                                    .SingleOrDefault();
            
            Console.WriteLine($"The logest song is - {longestSongName.Name}, with duration of {longestSong} seconds.");
            

            // 4- whats the total Duration of all Songs
            Console.WriteLine("======= Total duration of all songs======================================");
            var totalDuration = songsDuration
                                        .Sum();
            Console.WriteLine($"Total duration of all songs is- {totalDuration} seconds.");

            // 5- how many albums have Songs longer than 300 seconds
            Console.WriteLine("======= Albums with Songs longer than 300s ======================================");
            var songsLongerThan300 = Songs
                                        .Where(s => s.Duration > 300)
                                        .GroupBy(s => s.AlbumId)
                                        .ToList()
                                        .Count;
            Console.WriteLine($"There is a {songsLongerThan300} albums with songs longer than 300s.");
  
            // 6- print the names of the artists(separated with "--"), that have more than one album of PopRock genre
            Console.WriteLine("4.======= Artist's names, wich have more than one album of PopRock genre  ======================================");
            var popRockGenre = Artists
                                    .Where(art => art.Albums.Where(alb => alb.Genre == Genre.PopRock).ToList().Count() > 1)
                                    .Select(name => name.FullName)
                                    .ToList();
            popRockGenre.ForEach(one => Console.WriteLine($"{one} --"));

          
            // 7- print the name of the album that has highest Average duration of a song
            Console.WriteLine("5.======= Album with the highest averge duration of songs ======================================");
            var heighesAvergeAlbum = Albums
                                        .OrderBy(alb => alb.Songs.Select(s => s.Duration)
                                        .Average())
                                        .ToList()
                                        .FirstOrDefault();
            Console.WriteLine($"{heighesAvergeAlbum.Name} is an album with the heighest song averge.");

                                            
            // 8- how many characters has the song that has the shortest Duration
            Console.WriteLine("6.======= Number of characters of the shortest song  ======================================");
            var shortestSong = Songs
                                .OrderByDescending(s => s.Duration)
                                .ToList()
                                .LastOrDefault();
            Console.WriteLine($"{shortestSong.Name} - {shortestSong.Name.Count()} characters.");

   
            // 9- print the name and the genre of the album that has most songs
            Console.WriteLine("7.======= Genre of the album with the most songs ======================================");
            var albumWithMostSongs = Albums
                                         .OrderBy(alb => alb.Songs.Count())
                                         .ToList()
                                         .LastOrDefault();
            Console.WriteLine($"Album with the most songs is {albumWithMostSongs.Name} and the genre of the album is {albumWithMostSongs.Genre}.");
                                            
            // - print the name of the artist that has most songs
            Console.WriteLine("8.======= Artist with the most songs ======================================");
            var artistWithMostSongs = Artists
                                        .OrderBy(art => art.Albums.Select(s => s.Songs).Count())
                                        .Select(name => name.FullName)
                                        .FirstOrDefault();
            Console.WriteLine($"Artist with the most songs is {artistWithMostSongs}.");
            

            // - print the type of the artist(SoloArtist/Band) that has most albums published before year 2000
            Console.WriteLine("9.======= Artist with the most albums before 2000y ======================================");
            
            var artistWithMostAlbums = Artists
                                            .OrderBy(art => art.Albums.Where(alb => alb.Year < 2000).Count())
                                            .Select(a => a.ArtistType)
                                            .LastOrDefault();
            var artistWithMostAlbumsName = Artists
                                            .OrderBy(art => art.Albums.Where(alb => alb.Year < 2000).Count())
                                            .Select(a => a.FullName)
                                            .LastOrDefault();

            Console.WriteLine($"{artistWithMostAlbumsName} is {artistWithMostAlbums} with the most albums before 2000.");


            // - print the average song duration, of the album that has most songs
            Console.WriteLine("10. ======= Avarage song duration of album with most songs ======================================");
            
            var avDurAlbMostSongs = albumWithMostSongs.Songs
                                            .Select(s => s.Duration)
                                            .Average();
            
            Console.WriteLine($"Average duration of the album with the most songs is - {avDurAlbMostSongs}s.");

            // Bonus:
            // - print the longest song duration of the album that has least songs
            Console.WriteLine("======= Longest song duration of the album with the least songs ======================================");
            var albumWithLeastSongs = Albums
                                         .OrderBy(alb => alb.Songs.Count())
                                         .ToList()
                                         .FirstOrDefault();
            var longestSongDur = albumWithLeastSongs.Songs
                                                    .Select(s => s.Duration)
                                                    .LastOrDefault();
            Console.WriteLine($"{albumWithLeastSongs.Name} is album with the least songs and his longest duration song is {longestSongDur}s.");

            // - print the name of the album that has most songs that contain letter 'a' in the name
            Console.WriteLine("======= Album with the most songs that contain letter -a ======================================");
            
            var nameOfTheAlbum = Albums
                                .Where(alb => alb.Songs
                                .Where(s => s.Name.Contains("a")).Count() == Albums
                                .Select(a => a.Songs
                                .Where(so => so.Name.Contains("a")).Count())
                                .Max()).Select(al => al.Name)
                                .FirstOrDefault();
            var albMostSongContA = Albums
                                        .Select(alb => alb.Songs.Where(s => s.Name.Contains("a")).Count()).Max();


            Console.WriteLine($"Name of the album is {nameOfTheAlbum}, and has {albMostSongContA} songs contains letter -a.");
            // - print the name of the artist that has most songs that end with letter 'd'
            Console.WriteLine("======= Name of the artist that has the most songs ends with letter -d ======================================");
            var nameOfTheArtist = Artists
                                   .Where(ar => ar.Albums
                                   .Select(al => al.Songs
                                   .Where(s => s.Name.EndsWith("d")).Count()).Sum() == Artists
                                   .Select(art => art.Albums
                                   .Select(alb => alb.Songs
                                   .Where(so => so.Name.EndsWith("d")).Count()).Sum()).Max()).Select(a => a.FullName)
                                   .FirstOrDefault();
            var albMostSongContD = Albums
                                        .Select(alb => alb.Songs.Where(s => s.Name.Contains("d")).Count()).Max();

            Console.WriteLine($"Name of the artist is {nameOfTheArtist}, and has {albMostSongContD} songs ends with letter -d.");
